...
import { ArrowLeft, Trash2, Printer, Phone } from 'lucide-react'
...
export default function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <div className="flex items-center justify-center min-h-[400px] text-gray-400 text-sm">Loading receipt details…</div>
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="no-print flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-gray-200">
        ...
        <div className="flex flex-wrap gap-2 w-full sm:w-auto">
          ...
          <button
            ...
            className="flex items-center justify-center gap-1.5 px-3 py-2 border border-rose-250 text-rose-600 hover:text-rose-700 hover:bg-rose-50 text-xs rounded-xl transition-all font-medium"
          >
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 pt-4 border-t border-gray-200 print-border">
            ...
            <div className="md:col-span-5 space-y-2 text-xs text-gray-500 print-text-dark">
              ...
              {/* Outstanding dues replaced by premium PAID stamp */}
              <div className="flex justify-between items-center bg-green-50 text-green-700 border border-green-200 rounded-lg px-3 py-1.5 text-xs font-bold mt-2">
                <span>Payment Mode: {invoice.paymentMode}</span>
                <span className="tracking-wider uppercase text-[10px] bg-green-600 text-white px-2 py-0.5 rounded shadow-sm">Paid in Full</span>
              </div>
            </div>
          </div>
