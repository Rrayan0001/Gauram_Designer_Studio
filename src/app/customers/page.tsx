    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Billing history and client ledger</p>
        </div>
        ...
        <div className="p-4 border-b border-gray-100">
          <div className="relative max-w-sm">
            ...
            <input type="text" placeholder="Search by name or phone…" value={searchTerm}
              ...
              className="w-full pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-gray-400 bg-white"
              style={{ paddingLeft: '2.5rem' }} />
          </div>
        </div>

        {/* Desktop Table (Hidden on Mobile, simplified columns) */}
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50 text-left">
                <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Client</th>
                <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider text-center">Orders Count</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="text-center py-12">
                    <div className="flex flex-col items-center gap-2 text-gray-400">
                      ...
                      <p className="text-xs">Customers are created automatically when you generate a bill</p>
                    </div>
                  </td>
                </tr>
              ) : customers.map(c => (
                <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right font-bold text-gray-900">{fmt(c.totalBilled)}</td>
          ...
          {loading ? (
            ...
            customers.map(c => (
              <div key={c.id} className="p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs bg-gray-50 p-2.5 rounded-lg text-center">
                  <div>
                    <span className="block text-[9px] text-gray-400 uppercase tracking-wide">Orders Count</span>
                  ...
                  <div>
                    ...
                    <span className="font-bold text-gray-900">{fmt(c.totalBilled)}</span>
                  </div>
                </div>
